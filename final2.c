...
/////////
//README : 함수 설명은 아래 함수 정의 위(옆)에 있음요!
/////////
...
//QnA기능에서 사용자가 QnA를 달고, 답변을 확인하도록 한 함수
int question(QNA **qna, int *qna_n, int member_n){
    char temp[251]; 
    int len;
    int sel;
...
//QnA기능에서 사용자가 등록한 질문에 관리자가 답하고 확인할 수 있도록 하는 함수
int answer(QNA **qna, int *qna_n, int member_n){
    int i;
    int sel;
    int sel2;
...
//관리자가 모든 주문 내역을 확인하고, 상품의 매출, 주문량을 확인할 수 있도록 하는 함수
int product_statistic(PRO *pro, int pro_n, HIS *his, int his_n, CUS *cus, int cus_n){
...
//관리자가 상품을 등록,수정,삭제할 수 있도록 하는 함수
int addelete_product(PRO **pro, int *pro_n){
...
//관리자가 모든 회원들의 정보를 확인할 수 있도록 한 함수
int members_info(CUS *cus, int cus_n){
...
//회원정보가 담긴 csv파일로 부터 이를 담을 구조체 배열을 만드는 함수
...
//상품정보가 담긴 csv파일로 부터 이를 담을 구조체 배열을 만드는 함수
...
//배송관련 정보가 담긴 csv파일로 부터 이를 담을 구조체 배열을 만드는 함수
...
//QnA정보가 담긴 csv파일로 부터 이를 담을 구조체 배열을 만드는 함수
...
//회원정보가 저장된 구조체배열의 정보를 csv파일에 저장하는 함수
...
int init_file2(FILE **fp, PRO **pro, int pro_n){   // 상품정보가 저장된 구조체배열의 정보를 csv파일에 저장하는 함수
...
int init_file3(FILE **fp, HIS **his, int his_n){   // 배송정보가 저장된 구조체배열의 정보를 csv파일에 저장하는 함수
...
int init_file4(FILE **fp, QNA **qna, int qna_n){    //QnA가 저장된 구조체배열의 정보를 csv파일에 저장하는 함수
...
void buffer_clear(void){    //버퍼비우기 함수
    while(getchar() != '\n');
    return;
}
